...
The plugins can be in the system
  ...
  `gem env gemdir`/gems/bwkfanboy-x.y.z/lib/plugins
...
0. *bwkfanboy* script that takes 1 parameter: the name of a file in
   ...
   and it will load
   <tt>/usr/local/lib/ruby/gems/1.9/bwkfanboy-0.0.1/lib/plugins/bwk.rb</tt>
...
the permissions allows it.

== HTTP

There are 2 method to get an Atom feed by HTTP.

1. You can copy <tt>web/bwkfanboy.cgi</tt> (from the program tarball) to
   your Apache cgi directory and run it, but this disallows you from
   using your HOME directory for your own plugins.

2. Use small *bwkfanboy_server* HTTP server. If can run from any user
   and thus is able to inherit env variables for discovering your HOME
